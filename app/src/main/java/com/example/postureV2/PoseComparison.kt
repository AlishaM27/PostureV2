...
//data class ExerciseData(val angles: Map<String, Float>)
...
//This creates a list of float values
...
//            result.add(ExerciseData(angles))
        }
    }
    reader.close()
    return result
...
//    }
//
//    if (bestDeviation == Float.MAX_VALUE){
//        for (i in referenceFrames.indices){
//            val (deviation, jointDevs) = calculateFrameDeviation(currentFrame, referenceFrames[i])
//            if (deviation < bestDeviation){
//                bestDeviation = deviation
//                bestIndex =i
...
fun generateExerciseReport(performanceHistory: List<FrameMatchResult>, frameThreshold: Float = 35f, minSequenceLength: Int = 5): ExerciseReport {
    ...
    val totalDeviation = performanceHistory.sumOf {it.deviation.toDouble()}
    val avgDeviation = (totalDeviation / performanceHistory.size).toFloat()
    ...
    return ExerciseReport(avgDeviation, segments, score)
}
